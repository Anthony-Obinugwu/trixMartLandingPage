...
function BenefitsCard({directionIsLeft=false, benefactor, benefitsList}: BenefitsProps){
  return (
    <div className="relative px-10 sm:static sm:px-0">
          <div className={`${directionIsLeft ? "bg-[#FF8B2A1F]" : "bg-[#006ED30D]"} h-[400px] mx-auto px-6 rounded-xl sm:relative sm:flex sm:items-center  ${directionIsLeft ? "sm:justify-start" : "sm:justify-end"} sm:px-6 sm:h-44 md:px-10`}>
            ...
            <div className={`absolute left-0 right-0 top-28 flex flex-col gap-y-6 sm:max-w-96 sm:top-auto  ${ directionIsLeft ? "sm:left-auto sm:right-10" : "sm:right-auto sm:left-10" } md:max-w-none lg:min-w-[550px]`}>
              {benefitsList.map((benefit)=>(
                <div className={` ${directionIsLeft ? "bg-[#FFAD68]" : "bg-[#0A42B6]"} flex items-center py-4 px-4 rounded-xl`}>
                <Image src={`${directionIsLeft ? "icons/dark-check-mark.svg" : "icons/light-check-mark.svg"}`} className="mr-4" width={25} height={25} alt="checkmark" />
                <p className={`text-sm ${directionIsLeft ? "text-[#051650]" : "text-[#F1F1F1]"}`}>{benefit}</p>
              </div>
              ))}
            </div>
          </div>
